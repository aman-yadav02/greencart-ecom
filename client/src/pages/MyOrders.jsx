    ...
    return (
        <div className='mt-16 pb-16'>
            <div className='flex flex-col items-end w-max mb-8'>
                ...
                <div className="w-16 h-0.5 bg-[#4fbf8c] rounded-full"></div>
            </div>
                    ...
                    {order.items.map((item, index) => (
                        <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index +1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl `}>
                            <div className="flex-items-center mb-4 md:mb-0">
                                <div className="bg-[#4fbf8c]/10 p-4 rounded-lg">
                            ...
                            <div className='flex flex-col justify-center md:ml-30 mb-4 md:mb-0'>
                                <p>Quantity : {item.quantity || "1"}</p>
                                <p>Status : {order.status}</p>
                                <p>Date : {new Date(order.createdAt).toLocaleDateString()}</p>
                            </div>
                            <p className='text-[#4fbf8c] text-lg font-medium'>
